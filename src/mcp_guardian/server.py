...
# Self-contained hook script. Mirrors PATTERNS in spirit but stays
# independent so the hook works even if the Python package is not installed.
HOOK_SCRIPT = r"""#!/usr/bin/env bash
# mcp-guardian pre-commit hook — generated by `mcp-guardian install-hook`
# Mirrors the rules from src/mcp_guardian/patterns.py. Kept in sync manually.
